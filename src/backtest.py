...
def run_backtest(
    ...
    for index, row in close_data.iterrows():
        ...
        if operation == Operation.BUY and buy_ready == True and usdt > 0:
            ...
            myrow = write_operation(
                index,
                coin_name,
                Operation.BUY.value,
                Reason.BUY_MARKET.value,
                current_price,
                fee * current_price,
                usdt,
                coin,
            ...
            myrow = write_operation(
                index,
                coin_name,
                Operation.SELL.value,
                Reason.SELL_STOP_LOSS.value,
                current_price,
                fee,
                usdt,
                coin,
...
def backtest_evaluation(results: pd.DataFrame, close_dataframe: pd.Series) -> dict:
    ...
    data = {
        ...
        "avg_pct_negative_trades": round(
            dt.loc[dt["resultat"] < 0, "resultat%"].sum()
            / dt.loc[dt["resultat"] < 0, "resultat%"].count(),
            2,
        ),
    ...
    data["win_rate"] = (
        data["nb_positive_trades"]
        / (data["nb_negative_trades"] + data["nb_positive_trades"])
        * 100
    )
...
def multiple_coin_strategy(
    historical_data: dict,
    list_of_coin: List[str],
    usdt: float,
    stop_loss_pct: float,
    take_profit_pct: float,
    taker_fee: float,
    ...
    win_rate = (
        summary["nb_positive_trades"]
        / (summary["nb_negative_trades"] + summary["nb_positive_trades"])
        * 100
    )
